...
module.exports = class Application extends EventEmitter {
	constructor(options) {
		...
		// [to do ...] 增加新的触发器后，这段逻辑要变, [aliyun-http] 的元素为 request, response
	 ...
	 * 
	 * 实际上，完全可以做到兼容，但是调用层会变得异常麻烦(需要将主体代码写在 handler 之外)，得不偿失
	 */
	createContext() {
	...
	init() {
		...
		const parsedEvent = this.triggerModule.parseOriginEvent(this._originEvent)
