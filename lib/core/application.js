...
module.exports = class Application extends EventEmitter {
	...
	/**
	 * 这一段的用途是建立能够互相调用的关系，和原生 Koa 比较略微有点修改，主要是以下原因造成的：
	 * 在 Serverless 环境中，每 1 个请求都会创建一个新的 app 实例，对应 1 个 ctx
	 * 在原生 Koa 中，所有请求共用 1 个 app 实例，每 1 个请求会使用 1 个 ctx
	 */
	createContext() {
		...
		// 目前 request 和 req 是一个东西，不区分
		context.req = request.req = response.req = request

		// 目前 response 和 res 是一个东西，不区分
	...
	init() {
		// const ctx = this.context
		const parsedEvent = this.triggerModule.parseOriginEvent(this.request)
