...
module.exports = {
	...
	/**
	 * 状态码(statusCode)部分
	 * Serverless 中不需要状态消息(statusMessage)
	 */
	...
	set body(val) {
		...
		/**
		 * 备注：'Content-Type', 'Content-Length' 由 API 网关自动设置，无需人工指定
		 */
	 ...
	 */
	// set length(num) {
	// 	if (typeof num !== 'number' || num % 1 !== 0) throw new Error('参数错误: response 的 length 必须是一个整数')
	// 	this._length = num
	// 	this.setHeader('Content-Length', num);
	// },
