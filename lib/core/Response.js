'use strict'


const STATUS_CODES = require('../constant/status-codes')


class Response {
	constructor(ctx) {
		this.context = ctx
		this._headers = {}
		this._body

		// 默认状态码定义为 200
		this._statusCode = 200
		this._statusMessage = 'OK'
	}


	/**
	 * 消息头(headers)部分
	 */

	get headers() {
		return this._headers
	}

	set headers(val) {
		if (typeof val === 'object') {
			// 将字段名逐个改为小写后存入
			Object.keys(val).forEach(function (key) {
				this._headers[key.toLowerCase()] = val[key]
			})
		} else {
			throw new Error('参数错误: headers 应该是一个对象')
		}
	}

	// headers 别名 header
	get header() {
		return this.headers
	}

	set header(val) {
		this.headers = val
	}

	getHeader(field) {
		return this._headers[field.toLowerCase()] || ''
	}

	setHeader(field, value) {
		field = field.toLowerCase()
		this._headers[field] = value
	}

	hasHeader(field) {
		return field.toLowerCase() in this._headers
	}

	removeHeader(field) {
		delete this._headers[field.toLowerCase()]
	}

	// getHeader 别名 get
	get(field) {
		return this.getHeader(field)
	}

	// setHeader 别名 set
	set(field, value) {
		return this.setHeader(field, value)
	}

	// hasHeader 别名 has
	has(field) {
		return this.hasHeader(field)
	}

	// removeHeader 别名 deleteHeader
	deleteHeader(field) {
		return this.removeHeader(field)
	}

	// removeHeader 别名 remove
	remove(field) {
		return this.removeHeader(field)
	}


	/**
	 * 状态码(statusCode)部分
	 */

	get statusCode() {
		return this._statusCode
	}

	set statusCode(code) {
		// 如果 code 不是整数则报错
		if (typeof code !== 'number' || code % 1 !== 0) throw new Error('参数错误: response 的 statusCode 必须是一个整数')

		this._statusCode = code

		// 同时设置对应的状态消息(statusMessage)
		this._statusMessage = STATUS_CODES[code] || 'Unknown Code'
	}

	// statusCode 别名 status
	get status() {
		return this.statusCode
	}

	set status(code) {
		this.statusCode = code
	}

	status(code) {
		this.statusCode = code
	}


	/**
	 * 状态消息(statusMessage)部分
	 */

	get statusMessage() {
		return this._statusMessage
	}

	set statusMessage(val){
		// statusMessage 不允许自发设置
		return
	}

	// statusMessage 别名 message
	get message(){
		return this.statusMessage
	}

	set message(val){
		this.statusMessage = val
	}


	/**
	 * body 部分
	 */
	
	get body() {
		return this._body
	}

	set body(val) {
		this._body = val

		// to do ...
	}

	toJSON() {
		return {
			statusCode,
			statusMessage,
			headers,
			body,
		}
	}


}