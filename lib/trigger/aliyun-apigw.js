...
function parse(event) {
	...
	const ipAlias = ['ip', 'CaClientIp', 'clientIp', 'client_ip', 'clientip', 'ClientIp', 'ips']
	const hostnameAlias = ['domain', 'hostname', 'CaDomain', 'Domain', 'DomainName', 'domain_name']    // 不要使用 host, 可能会引起冲突
	...
	Object.keys(obj.headers).forEach(function (key) {
		...
		} else if (hostnameAlias.includes(key)) {
			obj.hostname = obj.headers[key]
			delete obj.headers[key]
		} else if (idAlias.includes(key)) {
			obj.id = obj.headers[key]
			delete obj.headers[key]
		} else if (protocolAlias.includes(key)) {
			obj.schema = obj.headers[key].toLowerCase()
			delete obj.headers[key]
